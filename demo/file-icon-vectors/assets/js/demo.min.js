...
(function($, undefined) {
  ...
  var _icons = $( '#icon_container', _container );
    ...
    getIcons: function( _set ) {
      ...
      $.get( url_prefix + 'dist/icons/' + _set + '/catalog.json' )
        .done(function( icons ) {
          ...
          var icon_html = '<div class="pure-u-1-2 pure-u-sm-1-3 pure-u-md-1-8"><span class="fiv-%s fiv-icon-%s"></span><p class="icon-desc">%s</p></div>';
          for ( var idx = 0; idx < icons.length; idx++ ) {
            _icons.append( $.sprintf( icon_html, ICON_NS.Core.icons_sets[_set]['prefix'], icons[idx], icons[idx] ) );
            //console.log( icons[idx] );
          }
